...
SELECT (SYSDATE - HIRE_DATE)/7 AS WEEK FROM EMPLOYEES; --주
SELECT (SYSDATE - HIRE_DATE)/365 AS YEAR FROM EMPLOYEES; --년
SELECT TRUNC((SYSDATE-HIRE_DATE)/365)*12 AS MONTH FROM EMPLOYEES; --달

--날짜의 반올림, 절삭
 SELECT ROUND(SYSDATE) FROM DUAL;
 SELECT ROUND(SYSDATE, 'DAY') FROM DUAL; --해당 주의 일요일로 반올림
